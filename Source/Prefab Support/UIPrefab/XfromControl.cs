...
public class XfromControl : MonoBehaviour {
    public Toggle T, R, S;
    public SliderWithEcho X, Y, Z, rotationSpeed;
	...
	void Start () {
        ...
        rotationSpeed.SetSliderListener(SpeedValueChanged);
    ...
    void SpeedValueChanged(float v)
    {
        UpdateSpeed(TheRoot, v);
    }

    void UpdateSpeed(SceneNode node, float v)
    {
        foreach (NodePrimitive np in node.PrimitiveList)
        {
            SetSpeed(np, v);
        }
